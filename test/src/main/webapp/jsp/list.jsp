...
<div class="container">
    ...
    <div style="float: left;">
        ...
        <form class="form-inline" method="post" action="${pageContext.request.contextPath}/product/findAll">
            <div class="form-group">
                <label for="exampleInputName2">商品名称</label>
                <input type="text" name="name" class="form-control" id="exampleInputName2" value="${name}">
            </div>
            <div class="form-group">
                <label for="exampleInputName3">商品价格</label>
                <input type="text" name="address" class="form-control" id="exampleInputName3" value="${price}">
            </div>

            <div class="form-group">
                <label for="exampleInputEmail2">生产日期</label>
                <input type="text" name="email" class="form-control" id="exampleInputEmail2" value="${date}" >
            </div>
            <div class="form-group">
                <label for="exampleInputEmail2">商品描述</label>
                <input type="text" name="email" class="form-control" id="exampleInputEmail4" value="${description}" >
            </div>
    ...
    <table border="1" class="table table-bordered table-hover">
        ...
        <c:forEach items="${goodsList}" var="goodsList" varStatus="s">
            <tr>
                <td><input type="checkbox" ></td>
                <td>${s.count}</td>
                <td>${goodsList.name}</td>
                <td>${goodsList.price}</td>
                <td>${goodsList.data}</td>
                <td>${goodsList.says}</td>
                <td><a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/goods/findById?id=${goodsList.id}">修改</a>&nbsp;
